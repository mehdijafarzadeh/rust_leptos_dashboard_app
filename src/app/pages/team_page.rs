...
pub fn TeamPage() -> impl IntoView {
    ...
    class = "bg-gray-900 overflow-x-hide" >
        ...
        <Header/>
           <div class="mt-20">
                <div class="text-white flex flex-col w-full mx-auto items-center justify-center align-center">
                    <div class="flex flex-row w-full max-w-[25rem]">
        
                        <div class="pr-4 mt-4 text-xl">"Members"</div> 
      <hr class="w-full max-w-[48rem] pl-4 pr-4 pt-4 mt-8 mr-4" />
        <button on:click=on_click class=ADD_BUTTON_STYLE>"Add"</button>
                    </div>
                </div>
            </div>
        < / div >
